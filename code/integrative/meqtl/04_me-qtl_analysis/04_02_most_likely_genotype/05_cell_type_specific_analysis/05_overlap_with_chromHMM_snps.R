...
GetVennDiagram <- function(blood.type, chromhmm.snps.df, dex.blood.types.snps.df, veh.blood.types.snps.df, chromhmm.state){
  ...
  plot.title = paste0(paste0("Number of intersecting SNPs in ", blood.type, " in ", chromhmm.state, "\n"))
  
  cbPalette <- c("darkgrey", "cadetblue", "skyblue")
  
  ggVennDiagram(snp.lst, 
                category.names = c(paste0(perc.olap.dex.chromhmm, " - ChromHM - ", perc.olap.veh.chromhmm), 
                                   paste0("dex\n", perc.olap.dex.dnam),
                                   paste0("base\n", perc.olap.veh.dnam)), 
                set_size = 4, 
                label_alpha = 0.7,
...
## Load data
## 
dex.blood.types.snp.lst <- readRDS("~/bio/code/mpip/dex-stim-human-array/output/data/integrative/cell_type_enrichment/dex_meqtl_snps_by_blood_cell_type_significant.rds")
...
chromhmm.snps.df <- chromhmm.snp.state.lst.df$`2_TssAFlnk`
chromhmm.snps.df <- left_join(chromhmm.snps.df, epigenome.blood.map.df[, .(EID, Salas)], by = c("eid" = "EID"))

blood.cell.types.of.interest <- c("CD4nv", "Neu")
...
grid.arrange(arrangeGrob(ggvenn.lst[[1]], ggvenn.lst[[2]],  widths = c(4, 4), nrow = 1)) 

# Tx
# 
chromhmm.state <- "Tx"

chromhmm.snps.df <- chromhmm.snp.state.lst.df$`4_Tx`
chromhmm.snps.df <- left_join(chromhmm.snps.df, epigenome.blood.map.df[, .(EID, Salas)], by = c("eid" = "EID"))

blood.cell.types.of.interest <- c("Neu")
...
chromhmm.snps.df <- chromhmm.snp.state.lst.df$`5_TxWk`
chromhmm.snps.df <- left_join(chromhmm.snps.df, epigenome.blood.map.df[, .(EID, Salas)], by = c("eid" = "EID"))

blood.cell.types.of.interest <- c("Bmem+Bnv", "CD4mem")
...
chromhmm.snps.df <- chromhmm.snp.state.lst.df$`6_EnhG`
chromhmm.snps.df <- left_join(chromhmm.snps.df, epigenome.blood.map.df[, .(EID, Salas)], by = c("eid" = "EID"))

blood.cell.types.of.interest <- c("Bmem+Bnv", "Neu")
...
chromhmm.snps.df <- chromhmm.snp.state.lst.df$`7_Enh`
chromhmm.snps.df <- left_join(chromhmm.snps.df, epigenome.blood.map.df[, .(EID, Salas)], by = c("eid" = "EID"))
