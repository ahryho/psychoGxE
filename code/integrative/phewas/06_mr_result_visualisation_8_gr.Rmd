...
mr.res$cpg        <- unlist(strsplit(mr.res$exposure, "_"))[seq(from = 4, to = 4*nrow(mr.res), by = 4)] 
...
```{r}
table(mr.res.ao.fdr.sig[trait_category == "Neuro-behavioral", blood_type])
```

```{r}
blood <- "Bmem"
mr_res_forest_plt <- mr.res.ao.fdr.sig[trait_category == "Neuro-behavioral" & blood_type == blood]
# mr_res_forest_plt <- left_join(mr_res_forest_plt, 
#                                mr.res[, c("exposure.group", "SNP", "cpg", "outcome", "id.exposure", "id.outcome")], 
#                                by = c("exposure.group", "SNP", "cpg", "outcome")) %>% 
#   left_join(ao[, c("id", "nsnp")], by = c("id.outcome" = "id") )
# plt <- my_mr_forest_plot(mr_res_forest_plt)
# plt[[1]]

mr_res_forest_plt <- generate_odds_ratios(mr_res_forest_plt)
summary(mr_res_forest_plt)

ggplot(mr_res_forest_plt, aes(paste(SNP, cpg, sep = " - "), or, colour = exposure_super_group))+
...
mr_res_forest_plt$or <- signif(mr_res_forest_plt$or, 2)
mr_res_forest_plt$b <- signif(mr_res_forest_plt$b, 2)
mr_res_forest_plt$se <- signif(mr_res_forest_plt$se, 2)
mr_res_forest_plt$or_lci95 <- signif(mr_res_forest_plt$or_lci95, 2)
mr_res_forest_plt$or_uci95 <- signif(mr_res_forest_plt$or_uci95, 2)
mr_res_forest_plt$fdr <- signif(mr_res_forest_plt$fdr, 2)

tabletext <- cbind(c("Otcome", substr(mr_res_forest_plt$outcome, 1, 5)),
                   c("SNP", mr_res_forest_plt$SNP),
                   c("CpG", mr_res_forest_plt$cpg),
                   c("beta", mr_res_forest_plt$b),
                   c("SE", mr_res_forest_plt$se),
                   c("fdr", mr_res_forest_plt$fdr),
                   c("OR [95% CI]", paste(mr_res_forest_plt$or, "[", mr_res_forest_plt$or_lci95, ";", mr_res_forest_plt$or_uci95, "]")))

forestplot::forestplot(labeltext = tabletext, 
                       mean = c(NA, mr_res_forest_plt$or), 
                       lower = c(NA, mr_res_forest_plt$or_lci95), 
                       upper = c(NA, mr_res_forest_plt$or_uci95), 
                       new_page = T,
           xlog = F, 
...
chisq.test(cross_table, simulate.p.value = F)
```
...
cnt.df <- mr.res.ao.fdr.sig[trait_category != ""] %>% 
  group_by(blood_type, trait_category, exposure_super_group) %>%
  tally() %>% 
  group_by(blood_type) %>% 
...
exposure.group_proportions <- mr.res.ao.fdr.sig[, .(blood_type, exposure_super_group)] %>%
  group_by(blood_type, exposure_super_group) %>% 
  tally()%>%
  group_by() %>%
  dplyr::mutate(freq = round(n / sum(n), 3))

ggplt_bl_type <- lapply(unique(cnt.df$blood_type), function(blood){
  ggplot(cnt.df[blood_type == blood], 
         aes(x = trait_category, y = freq, fill = exposure_super_group, color = exposure_super_group)) +     
    geom_bar(stat = "identity", position = "dodge") +
    geom_hline(data = exposure.group_proportions[exposure.group_proportions$blood_type == blood, ], 
               color = cbPalette,
               aes(yintercept = freq)) +
    # geom_text(data = cnt.df[blood_type == blood], #[freq > 0.01], 
    #           aes(label = scales::percent(freq, accuracy = 1)),
    #           position = position_dodge(width = -0.9), size = 3, color = "black") +
  # facet_wrap(.~blood_type, nrow = 4) +
  theme_minimal() +
  theme(legend.title = element_blank(), 
        legend.position = "bottom",
        legend.text = element_text(size = plt.size, color = "black"),
        panel.grid.major = element_line(alpha("grey", 0.25)), 
        panel.grid.minor = element_line(alpha("grey", 0.25)),
        panel.background = element_blank(),
        axis.text.x = element_text(angle = 20, hjust = 0.5, size = plt.size, colour = "black"),
        axis.text.y = element_text(size = plt.size, color = "black"),
    ...
    labs(x = "",
         y = paste(blood, "SNP-CpG Proportion"))
})
```
...
frequent.traits <- mr.res.ao.fdr.sig[trait_category == "Neuro-behavioral",] %>%
   group_by(blood_type, exposure_super_group, trait) %>%
   summarize(n_trait = n()) %>%
   arrange(desc(n_trait)) %>%
   filter(trait %in% head(trait, 7)) %>%
   ungroup() %>% mutate(exposure_super_group = factor(exposure_super_group))

text.size <- 10

   frequent.traits %>% mutate(trait = factor(trait, levels = rev(unique(trait)))) %>% 
     ggplot(aes(x = trait, y = n_trait)) +
        geom_bar(aes(fill = exposure_super_group, color = exposure_super_group), 
...
```{r}
snp_hits_category_plots <- get_snp_hits_per_category_plot(mr.res.ao.fdr.sig, bl_type = "Bmem")
