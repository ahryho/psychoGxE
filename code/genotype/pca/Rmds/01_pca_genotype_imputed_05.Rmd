***
## The data from DC with window = 50 and shift  = 5
***
...
```{sh, eval = F}
plink --bfile qc/subset/dex_imputed_bggt_all_qc --indep-pairwise 50 5  0.2 --out ld_50_5/ld/dex_geno_imputed_ld
```
...
Options in effect:
  ...
  --out ld_50_5/ld/dex_geno_imputed_ld

64168 MB RAM detected; reserving 32084 MB for main workspace.
3936233 variants loaded from .bim file.
293 people (0 males, 0 females, 293 ambiguous) loaded from .fam.
Ambiguous sex IDs written to ld_50_5/ld/dex_geno_imputed_ld.nosex .
Using 1 thread (no multithreaded calculations invoked).
Before main variant filters, 293 founders and 0 nonfounders present.
Calculating allele frequencies... done.
...
3936233 variants and 293 people pass filters and QC.
Note: No phenotypes present.
Pruned 293557 variants from chromosome 1, leaving 12625.
Pruned 328431 variants from chromosome 2, leaving 13198.
Pruned 279668 variants from chromosome 3, leaving 11550.
Pruned 282014 variants from chromosome 4, leaving 10891.
Pruned 257390 variants from chromosome 5, leaving 10574.
Pruned 247684 variants from chromosome 6, leaving 10478.
Pruned 219982 variants from chromosome 7, leaving 9199.
Pruned 210668 variants from chromosome 8, leaving 8517.
Pruned 164505 variants from chromosome 9, leaving 7297.
Pruned 205350 variants from chromosome 10, leaving 8798.
Pruned 200231 variants from chromosome 11, leaving 8696.
Pruned 187115 variants from chromosome 12, leaving 8170.
Pruned 150378 variants from chromosome 13, leaving 6385.
Pruned 126560 variants from chromosome 14, leaving 5609.
Pruned 100292 variants from chromosome 15, leaving 4642.
Pruned 96525 variants from chromosome 16, leaving 4591.
Pruned 84244 variants from chromosome 17, leaving 4252.
Pruned 107175 variants from chromosome 18, leaving 4877.
Pruned 64048 variants from chromosome 19, leaving 3334.
Pruned 73451 variants from chromosome 20, leaving 3598.
Pruned 52358 variants from chromosome 21, leaving 2395.
Pruned 42666 variants from chromosome 22, leaving 2265.
Pruning complete.  3774292 of 3936233 variants removed.
Marker lists written to ld_50_5/ld/dex_geno_imputed_ld.prune.in and
ld_50_5/ld/dex_geno_imputed_ld.prune.out .
```

2. Generate genome file for MDS

```{sh, eval = F}
plink --bfile qc/subset/dex_imputed_bggt_all_qc --extract ld_50_5/ld/dex_geno_imputed_ld.prune.in --make-bed --out ld_50_5/pca/ld_extracted/dex_geno_imputed_ld
```
...
Options in effect:
  --bfile qc/subset/dex_imputed_bggt_all_qc
  --extract ld_50_5/ld/dex_geno_imputed_ld.prune.in
  --make-bed
  --out ld_50_5/pca/ld_extracted/dex_geno_imputed_ld

64168 MB RAM detected; reserving 32084 MB for main workspace.
3936233 variants loaded from .bim file.
293 people (0 males, 0 females, 293 ambiguous) loaded from .fam.
Ambiguous sex IDs written to ld_50_5/pca/ld_extracted/dex_geno_imputed_ld.nosex
.
--extract: 161941 variants remaining.
Using 1 thread (no multithreaded calculations invoked).
Before main variant filters, 293 founders and 0 nonfounders present.
Calculating allele frequencies... done.
Total genotyping rate is 0.995116.
161941 variants and 293 people pass filters and QC.
Note: No phenotypes present.
--make-bed to ld_50_5/pca/ld_extracted/dex_geno_imputed_ld.bed +
ld_50_5/pca/ld_extracted/dex_geno_imputed_ld.bim +
ld_50_5/pca/ld_extracted/dex_geno_imputed_ld.fam ... done.
```

### __MDS with PLINK__

```{sh, eval = F}
plink --bfile ld_50_5/pca/ld_extracted/dex_geno_imputed_ld  --cluster --mds-plot 293 'eigvals' 'eigendecomp' --out ld_50_5/pca/mds/dex_geno_imputed_ld_mds
```
...
```{yaml}
Options in effect:
  --bfile ld_50_5/pca/ld_extracted/dex_geno_imputed_ld
  --cluster
  --mds-plot 293 eigvals eigendecomp
  --out ld_50_5/pca/mds/dex_geno_imputed_ld_mds

64168 MB RAM detected; reserving 32084 MB for main workspace.
161941 variants loaded from .bim file.
293 people (0 males, 0 females, 293 ambiguous) loaded from .fam.
Ambiguous sex IDs written to ld_50_5/pca/mds/dex_geno_imputed_ld_mds.nosex .
...
Clustering... done.
Cluster solution written to ld_50_5/pca/mds/dex_geno_imputed_ld_mds.cluster1 ,
ld_50_5/pca/mds/dex_geno_imputed_ld_mds.cluster2 , and
ld_50_5/pca/mds/dex_geno_imputed_ld_mds.cluster3 .
Performing multidimensional scaling analysis (eigendecomposition algorithm,
293 dimensions)... done.
MDS solution written to ld_50_5/pca/mds/dex_geno_imputed_ld_mds.mds
(eigenvalues in ld_50_5/pca/mds/dex_geno_imputed_ld_mds.mds.eigvals ).
```
...
```{sh, include = F, eval = F}
scp -C -r -o ProxyCommand="ssh cluser@biocomp.psych.mpg.de nc slurmgate 22" ahryhorzhevska@slurmgate:/binder/mgp/workspace/2020_DexStim_Array_Human/snps/imputed_qc/from_darina/ld_50_5 .
```

```{r, include = F, eval = T}
eigenvec.fn  <- "~/bio/code/mpip/dex-stim-human-array/data/snps/imputed_qc/from_darina/ld_50_5/pca/mds/dex_geno_imputed_ld_mds.mds"
eigenval.fn  <- "~/bio/code/mpip/dex-stim-human-array/data/snps/imputed_qc/from_darina/ld_50_5/pca/mds/dex_geno_imputed_ld_mds.mds.eigvals"
